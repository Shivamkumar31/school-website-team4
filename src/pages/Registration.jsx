...
const Registration = () => {
  ...
  const validateFields = () => {
    ...
    if (selectedForm === "studentRegister" || selectedForm === "studentAdmission") {
      if (!student.firstName.trim()) validationErrors.firstName = "First name is required.";
      if (!student.lastName.trim()) validationErrors.lastName = "Last name is required.";
      ...
      if (!student.parentPhone.trim()) validationErrors.parentPhone = "Parent phone is required.";
    }
    if (selectedForm === "studentAdmission") {
      if (!student.bloodType) validationErrors.bloodType = "Blood group is required.";
      if (!student.admissionDate) validationErrors.admissionDate = "Admission Date is required.";
      if (!student.fatherName.trim()) validationErrors.fatherName = "Father’s name is required.";
      if (!student.motherName.trim()) validationErrors.motherName = "Mother’s name is required.";
      if (!student.parentEmail.trim()) validationErrors.parentEmail = "Parent’s email is required.";
      if (!student.permanentAddress.trim()) validationErrors.permanentAddress = "Permanent address is required.";
      if (!student.presentAddress.trim()) validationErrors.presentAddress = "Present address is required.";
      if (!student.profilePic) validationErrors.profilePic = "Profile picture is required.";
    }
    if (selectedForm === "studentLogin") {
      if (!student.rollNumber.trim()) validationErrors.rollNumber = "Roll number is required.";
      if (!student.classLevel.trim()) validationErrors.classLevel = "Class is required.";
    }
    if (selectedForm === "teacher") {
      if (!teacher.fullName.trim()) validationErrors.fullName = "Full name is required.";
      ...
      if (!teacher.qualification.trim()) validationErrors.qualification = "Qualification is required.";
      if (!teacher.subjectSpecialization.trim()) validationErrors.subjectSpecialization = "Subject specialization is required.";
      if (!teacher.joiningDate) validationErrors.joiningDate = "Joining date is required.";
      if (!teacher.phone.trim()) validationErrors.phone = "Phone number is required.";
      if (!teacher.email.trim()) validationErrors.email = "Email address is required.";
      if (!teacher.address.trim()) validationErrors.address = "Address is required.";
    }
...
{studentMenuOpen && !selectedForm && (
  <div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      >
        ...
        <h3 className="text-lg font-bold text-green-800 mt-2">Register Student</h3>
      ...
      >
        <span className="text-4xl">🏫</span>
        <h3 className="text-lg font-bold text-green-900 mt-2">Admit Student</h3>
      ...
      >
        <span className="text-4xl">🔑</span>
        <h3 className="text-lg font-bold text-yellow-800 mt-2">Student Login</h3>
        <p className="text-gray-500 mt-1">Already admitted? Login here</p>
      </motion.div>
    </div>
  ...
  {selectedForm === "studentRegister" && (
    <motion.form
      ...
      exit={{ opacity: 0, y: -50 }}
    >
      <h2 className="text-2xl font-bold text-green-600 mb-4">📝 Student Registration</h2>
...
{selectedForm === "studentLogin" && (
  <motion.form
    onSubmit={(e) => {
      e.preventDefault();
      alert(`🎉 Welcome Student! Roll No: ${student.rollNumber}, Class: ${student.classLevel}`);
      setSelectedForm("");
    }}
    className="mt-8 grid gap-5"
    initial={{ opacity: 0, y: 50 }}
    ...
    exit={{ opacity: 0, y: -50 }}
  >
    <h2 className="text-2xl font-bold text-yellow-600 mb-4">🔑 Student Login</h2>
  ...
  {selectedForm === "studentAdmission" && (
    <motion.form
      ...
      exit={{ opacity: 0, y: -50 }}
    >
      <h2 className="text-2xl font-bold text-green-600 mb-4">🏫 Student Admission</h2>

      {/* 🔹 Personal Information */}
      <div className="bg-green-50 rounded-xl p-4">
        <h3 className="text-lg font-bold mb-2 text-green-700">🔹 Personal Information</h3>
      ...
      <div className="bg-green-50 rounded-xl p-4">
        <h3 className="text-lg font-bold mb-2 text-green-700">🔹 Parent/Guardian Details</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block mb-1 font-medium">Father’s Occupation</label>
            <input
              type="text"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block mb-1 font-medium">Mother’s Occupation</label>
            <input
              type="text"
        ...
        {/* Parent’s Contact */}
        <div>
          <label className="block mb-1 font-medium">Parent’s Contact Number *</label>
          <input
            type="text"
            name="parentPhone"
            value={student.parentPhone}
        ...
        {/* Parent’s Email */}
        <div>
          <label className="block mb-1 font-medium">Parent’s Email Address *</label>
          <input
            type="email"
            name="parentEmail"
   ...
   {/* ✅ Teacher Form */}
          {selectedForm === "teacher" && (
            <motion.form
              ...
              exit={{ opacity: 0, y: -50 }}
            >
              <h2 className="text-2xl font-bold text-blue-600 mb-4">👩‍🏫 Teacher Registration</h2>
    ...
    <div>
      ...
      {errors.subjectSpecialization && (
        <p className="text-red-500 text-sm">{errors.subjectSpecialization}</p>
      )}
    </div>
